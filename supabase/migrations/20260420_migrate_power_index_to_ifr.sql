-- =============================================
-- Migration: Change Power Index to IFR (Índice de Força Relativa)
-- Modifies the KPI calculation to use relative strength normalized by body weight.
-- =============================================
...
SELECT jsonb_build_object(
    'kpis', jsonb_build_object(
      ...
      -- AJUSTE: IFR buscando apenas o MAIOR PR de cada exercício (Normalizado para Kg)
      'ifr', COALESCE((
        SELECT ROUND(
          (SUM(max_pr_kg) / NULLIF(p_user_weight, 0))::NUMERIC, 2)
        FROM (
          SELECT 
            exercise,
            MAX(
